...
def get_1(put_id):
    ...
    if mobitel_id <= len(mobiteli) or not 0:
        ...
        for mobitel in mobiteli:
            found_phone = mobitel if mobitel.get(
                'PkMobitel') == mobitel_id else found_phone
        file_name = mobitel.get('Wikilink')
        ...
        if found_phone:
            response = {
                ...
                "reponse": {
                    ...
                    "slika": '/mobiteli/marka/' + put_id + '/picture',
                    "links": [
                        {
                            "href": put_id + "/marka",
                            "rel": "marka",
            ...
            @app.route('/mobiteli/marka/<put_id>/picture')
            def img1(put_id):
                for check in cache:
                    timet = None
                    if timet == check.get("Timeout"):
                        help = {"Timeout": current_seconds_time()}
                        cache.append(help)

                if item:
                    timeout = item.get('Timeout')
                    if timeout < current_seconds_time():
                        return app.response_class(response=json.dumps(response), status=200,  headers={'Content-Type': 'image/jpeg', 'Content-Disposition': 'inline'})
                    cache = fetch_img(file_name)
                    image_url = cache.get("url")
...
def get_2(marka_id):
    ...
    for mobitel in mobiteli:
        found_phone = mobitel if mobitel.get(
            'Marka') == phone_id else found_phone
    ...
    response = {
        ...
        "reponse": {
            ...
            "slika": '/mobiteli/Naziv/' + marka_id + '/picture',
            "links": [
                {
                    "href": marka_id + "/naziv",
                    "rel": "naziv",
        ...
        if found_phone:
            response = {
                ...
                "reponse": {
                    ...
                    "slika": '/mobiteli/boje/' + put_id + '/picture',
                    "links": [
                        {
                            "href": put_id + "/boje",
                            "rel": "boje",
...
def get_4(naziv_id):
    ...
    for mobitel in mobiteli:
        found_phone = mobitel if mobitel.get(
            'Naziv') == phone_id else found_phone
    ...
    response = {
        ...
        "reponse": {
            ...
            "slika": '/mobiteli/datum/' + naziv_id + '/picture',
            "links": [
                {
                    "href": naziv_id + "/datum",
                    "rel": "datum",
        ...
        if found_phone:
            response = {
                ...
                "reponse": {
                    "id": put_id,
                    "Naziv": found_phone.get('Naziv'),
                    "slike": '/mobiteli/naziv/' + put_id + '/picture',
                    "links": [
                        {
                            "href": put_id + "/naziv",
                            "rel": "naziv",
...
def put_dimenzije(put_id):
    ...
    found_index = None
    for mobitel in mobiteli:
        found_index = mobiteli.index(mobitel) if mobitel.get(
            'PkMobitel') == int(put_id) else found_index
